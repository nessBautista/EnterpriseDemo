...
class RangedAxisExample: UIViewController {
    ...
    private func initChart(_ line1ModelData: [(Double, Double)], _ line2ModelData: [(Double, Double)],  _ line3ModelData: [(Double, Double)]) {
        ...
        let firstYear: Double = (line1ModelData.min(by: {$0.0 < $1.0}))?.0 ?? 0
        let lastYear: Double = (line1ModelData.max(by: {$0.0 < $1.0}))?.0 ?? 0
        ...
        let yModel = ChartAxisModel(lineColor: UIColor.white, firstModelValue: 0, lastModelValue: 200, axisTitleLabels: [], axisValuesGenerator: generator, labelsGenerator: labelsGenerator)
        ...
        let line2Model = ChartLineModel(chartPoints: line2ChartPoints, lineColor: .green, lineWidth: 2, animDuration: 1, animDelay: 0)
        
        let line3ChartPoints = line3ModelData.map{ChartPoint(x: ChartAxisValueDouble($0.0), y: ChartAxisValueDouble($0.1))}
        let line3Model = ChartLineModel(chartPoints: line3ChartPoints, lineColor: .red, lineWidth: 2, animDuration: 1, animDelay: 0)
        ...
        let originPoint = ChartPoint(x: ChartAxisValueDouble(0.0), y: ChartAxisValueDouble(0.0))
        let chartPointsLayer = ChartPointsViewsLayer(xAxis: xAxisLayer.axis, yAxis: yAxisLayer.axis, chartPoints: [line1ChartPoints.last ?? originPoint, line2ChartPoints.last ?? originPoint, line3ChartPoints.last ?? originPoint], viewGenerator: viewGenerator, mode: .custom, clipViews: false)
